...
**A review's verify stage spawns at most the cap `tools/hooks/agent-cap.js` resolves,
whatever the finding count.** The batch size grows
with the finding count; the agent count never does.

The NUMBER is a FILE CONSTANT in `tools/hooks/agent-cap.js`, and that file is what a run actually
...
- `tools/hooks/agent-cap.js` on the **`Workflow`** tool call — the STATIC half. It sees the inline
...
and a script that spells a pointer instead of an integer does not run. `tools/hooks/agent-cap.js` re-resolves
...
resolve to an integer within the cap `tools/hooks/agent-cap.js` holds is denied; the burden is on
the fan-out.

The cap is a FILE CONSTANT in `tools/hooks/agent-cap.js` — the one place it is written and the
...
- **Scale a large fresh surface by adding LENSES, not skeptics.** Coverage is what more agents buy;
  ...
  `tools/hooks/agent-cap.js` holds it as `MAX_LENSES` — it
