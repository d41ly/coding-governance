...
regular expression. `tools/check-wiring.sh` asserts that VALUE, because a group left at `Workflow`
...
- A `Workflow({name:'…'})` run of a saved workflow supplies no source to the hook. Those workflows
  live in `tools/workflows/` and the merge-bar leg covers them there.
...
spelling is paraphrased rather than quoted: this document renders into `tools/workflows/`, which the
