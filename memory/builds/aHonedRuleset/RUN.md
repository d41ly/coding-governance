...
2026-09-07T08:10:48Z decision · item the gates-green override recorded at close is SUPERSEDED by an actually green bar · reason The override was recorded on 2026-09-06 when 1 of 43 legs failed (drift-audit records) and both of its entries reproduced at BASE 6ec402bd. Landing then merged origin/main, 169 commits, and the picture changed in BOTH directions. The .lexicon.conf LANGS ratchet entry the override names was fixed on main by another build and is gone. The closed_specs_with_no_product_commit entry turned into a NEW failure that WAS this run's: the merge left two trace-waiver rows stale, because 36 product commit subjects now name aHonedRuleset and 38 name aSurfacedLexicon, so both specs are traceable and neither needs waiving. That was fixed at 877acc9f by reaping the two rows, which is the shrink-only direction the file declares. Checked rather than assumed: a clean checkout of origin/main at 5e16fc77 reports the signal at 1 against pin 1 and green, so the merged red was not inherited. The post-merge bar is now gates GREEN, 45/45 legs passed, 2 skipped as unchanged-vs-main and 49 self-tests held by the owner's mid-run directive. A reader who finds the parked gates-green override should read this entry beside it: the tree that lands is green, not overridden.
