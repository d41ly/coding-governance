...
for cls in ("CONTROL", "GRADED", "STABILITY-ONLY"):
    ...
    p = "/".join("%.2f" % agg(sel, "plain", "r@%d" % k)[0] for k in KS)
    tv = [agg(sel, "terms", "r@%d" % k) for k in KS]
...
for kind in ("graded-miss", "graded-binding"):
    ...
    print("  %-14s %4d   %-28s   %s" % ("..." + kind, n,
          "/".join("%.2f" % agg(sel, "plain", "r@%d" % k)[0] for k in KS),
          "/".join("%.2f" % agg(sel, "terms", "r@%d" % k)[0] for k in KS)))
...
print("%-16s %4d   %-28s   %s" % ("  ...overlap-ok", n,
      "/".join("%.2f" % agg(sel_clean, "plain", "r@%d" % k)[0] for k in KS),
      "/".join("%.2f" % agg(sel_clean, "terms", "r@%d" % k)[0] for k in KS)))
...
print("\ndeclared %s" % pin)
ctrl_r5, cn = agg(lambda q: q["class"] == "CONTROL", "plain", "r@5")
