...
- S1 — `map_lib.build_reference_index` — the present-layer set is recorded BEFORE the extension filter, inside the walk that already happens. The filter admits only extensions the symbol corpus already covers, so a layer with no extractor is invisible downstream of it; recording earlier is what makes the derivation possible with no second scan
