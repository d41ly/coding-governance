...
- AC24 — `build_run_model` (`test_model_ac24_read_only_visit`) — added at rev-17 by the fold of R2-M1
  of the closing diff review, round 2, and observed by it over synthetic fixtures before the arm
  existed: a second session's in-window `--status`, with three tool calls and an owner turn in a store
  extract, named no session of the run and left the model equal field for field to one built over a
  store holding only the run's own extract. With `READ_ONLY_VERBS` emptied the visitor joined and
  brought one owner turn and three calls. A foreign `--status` an hour after the run's last act moved
