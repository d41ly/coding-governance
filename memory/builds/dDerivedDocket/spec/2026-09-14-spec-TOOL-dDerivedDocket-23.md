...
**Status:** SPECCED · rev-1 · 2026-09-14 · node d · Tier-2 · base abac6d59 · streams tooling · order 23
...
| [2026-09-14-review-TOOL-dDerivedDocket-1-spec-audit-g4-round1.md](../reviews/2026-09-14-review-TOOL-dDerivedDocket-1-spec-audit-g4-round1.md) | spec-audit | TOOL-dDerivedDocket-21 TOOL-dDerivedDocket-24 TOOL-dDerivedDocket-25 TOOL-dDerivedDocket-26 TOOL-dDerivedDocket-29 TOOL-dDerivedDocket-30 TOOL-dDerivedDocket-31 |
...
- **S1** `GATE_ATTRIBUTE=<R>` on `tools/run-gates/run-gates.sh`. After the pool drains and after every
  ...
  in a detached scratch worktree at R, under R's ceiling for that row. Observed by AC1, AC3 and AC7.
...
- **S3** An optional manifest key `signature`: an argv, run in the tree being graded, that prints one
  line per offender and truncates nothing. It is declared on four legs: `lexicon naming predicates`
  and `install-prefix (shipped surface)` through their existing `--list` modes, and
  `drift-audit records` and `memory hygiene` through a new `--offenders` mode in each checker.
...
- **S8** `.githooks/pre-push` exports `GATE_ATTRIBUTE` set to the remote sha it reads for the default
  ...
  AC7.
- **S9** No version constant moves in this unit. The run-gates, drift-audit and memory-tree kits,
  whose shipped bytes S3 and S7 change, each move once in this build through another unit — the
  scratch-hygiene unit for run-gates, unit 21 for drift-audit — and this unit's bytes ride that move.
  NOT OBSERVED by a criterion here: `kit version markers` grades the final tree's constant-marker
  agreement, which is all it can grade.
...
manifest   "signature": ["python", "tools/lexicon/lexicon.py", "--list"]      optional, per row
...
record     <git-dir>/gate-run/<id>/attribution      one TSV row per red leg:
           <leg> <verdict> <inherited> <own> <R sha> <reason>
```
...
2. **CONTENDED** — the leg's recorded attempt ended 124 or 137 with a positive bound in its `.bound`
   file. Read from the attempt record, so a later retry pass that keeps the first attempt's row keeps
   this reading. Not re-run.
3. **OWN** — the leg has no row in R's manifest; or its argv differs from R's; or the diff between R
   and the working tree touches a tracked file in its argv, its comparator; or rc_R is 0.
4. **DEAD PROBE** — the leg cannot be run at R: the worktree cannot be made, R's argv file is absent,
   ...
   of anything (KF14).
5. **INHERITED or MIXED** — with a signature, S(X) is the signature's stdout lines at X after the
   normaliser; INHERITED when S(L) is non-empty and S(L) ⊆ S(R), else MIXED, whose `own` is
   |S(L) − S(R)|. Without one, S(X) is the normalised output; INHERITED only when the two are
   byte-identical, else MIXED. A new offender line that arrives with no FAIL line is therefore MIXED,
   never INHERITED — the direction KF14 ranked as the blocker.
...
- It runs after `remove_wall_watcher`, so the attribution pass is not inside the run's wall. Its own
  bound is the sum of the red legs' R ceilings, printed before it starts.

### The `--offenders` modes

`drift_report.py --offenders` prints `<signal><TAB><detail key>` for each gateable signal over its
pin and each ratchet finding, and exits as `--check` does. `check-memory-hygiene.sh --offenders`
prints `check <n><TAB><line>` for each line each failing check lists, and exits as the default mode
does. Neither changes the default output of either checker.
...
`tools/memory-tree/check-memory-hygiene.test.sh` · `tools/run-gates/README.md` · the run-gates dossier.
...
- **A signature as a regular expression over the leg's own output.** The gov canary prints the first
  ...
  test over a truncated S(L) reads a hidden new offender as inherited. A list mode truncates nothing.
...
- perf / scale — a red bar pays each red leg once more at R, plus one worktree. A green bar pays
  nothing. The pass sits outside the run's wall and prints its own bound.
...
- **AC3** — When the fixture leg's argv file does not exist at R, its line reads `DEAD PROBE` and the
  ...
  exit, it reads `DEAD PROBE` as well.
  Red when: a leg that cannot run at R is read as an empty set, so it reads INHERITED.
...
`run-gates canary` · `run-gates evidence` · `pre-push self-test` · `run-selftests self-test` · `drift-audit selftest` · `memory-hygiene self-test` · `testsuite counts (every bar self-test prints one)` · `install-prefix (shipped surface)` · `kit version markers` · `memory hygiene` · `spec tokens (a spec's own names resolve)`

New arm: tools/run-gates/run-gates.test.sh · a two-commit fixture with a signature leg, a comparator edit, a leg absent at R and a KF3 touch · the canary's executed-assertion floor, raised by the arms added
