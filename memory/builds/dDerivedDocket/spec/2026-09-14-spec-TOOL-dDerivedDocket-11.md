...
- **S11** A `--selftest` over fixture repositories with real history, printing
  ...
  Its ceiling is stated in `tools/gate-legs.json`, at or under the 300 s `DIRECT_CHECK_BOUND` the
...
- **hands-off** `TOOL-dDerivedDocket-12` — the module, the permissive legacy parser, the copy-choice
  rule, and the `--selftest` leg that the relocation engine extends, with the ceiling bound §8 F9
  sets; S7's names-an-id test, which that unit's classification table adopts for its unnamed-hold
  policy; and the signed-record header cells §4 pins, which that unit's `--signed` reads.
...
- **hands-off** `TOOL-dDerivedDocket-34` — the census's distinct-id count the conservation check
  ...
  substitutes the minted id (its S1 files it, and its AC3 compares with the substitution);
  `--plan` run over a worktree of the remote tip at the landing reconcile (S1); and the `--selftest`
  leg its `--write` fixtures extend, with the ceiling bound §8 F9 sets.
...
Over `fb07ca25`, 2026-09-16, node `d`: 658 distinct row ids across the four shards and three family
archives, 580 live row copies, 153 ids equal to a spec H1, 96 slugs owning rows of which 6 have no
build README, and 326 OPEN asks on 59 builds whose derived status is terminal, plus 14 OPEN asks in
README-less slugs that no build status covers. The row-lead count behind them reproduces every
figure rev-3 pinned at this build's records commit: 599, 521, 152, 91, 6, 288 on 56, and 14. PINNED
as that measurement; `--plan` re-derives every one of them and the switch-over reads its own run.
...
- perf / scale — three history walks regardless of the census size; everything else is in memory.
  The census is about 660 ids and 96 slugs at BASE.
...
- **AC10** — When `migrate_backlog.py --selftest` runs, it prints its `PASS (<n> assertions)` line at
  ...
  dossier, with a `ceiling` at or under 300, and the codebase-map coverage leg green;
  `bash tools/run-gates/run-selftests.sh --check` passes; and `python tools/govkit/govkit.py selfcheck`
  passes with the leg claimed by a `[[gate_leg]]` in `tools/memory-tree/kit.toml` and pinned in
  ...
  unpinned at the post-build bar; or its ceiling sits above `DIRECT_CHECK_BOUND`, which the
  ...
  permission: the `--selftest` run is the flag-form direct check the pass makes, which is why §8 F9
  keeps this leg's ceiling at the bound; `bash tools/run-gates/run-selftests.sh --check` and
  `python tools/govkit/govkit.py selfcheck` are gate-leg commands no pass runs (build method M6) and
  bind at the one post-build bar. Each arm's RED is observed by hand against a scratch fixture with
  its fix unstaged and recorded in the pass journal.
- **AC11** — When the unit's pass runs `--plan` over this repo with `--record` naming this build's
  ...
  cost: minutes, for the history walks.
  permission: the `--plan` run and the two-file count are the pass's own; unit passes run no gate
  (fix F7, and the child prompt in `tools/workflows/unattended-unit.js` since aProbedUnit), so the
  hygiene verdict over the four records is observed at the one post-build bar.
...
- **F9** — Where does the `backlog migration selftest` leg's ceiling sit against the direct-check
  bound? Since aDeferredBar, `tools/unattended/gate-guard.test.sh` grades every `chunk = selftests`
  leg, and reds one whose `--selftest` flag form carries a ceiling above its `DIRECT_CHECK_BOUND` of
  300 s unless that suite names the script as exempt. (a) A ceiling at or under the bound, so the
  flag stays the direct check a pass may run. (b) A ceiling above it, with `migrate_backlog.py` added
  to that suite's exemption list. (c) A `selftest.py` file entrypoint, which the hook denies. (b)
  edits an unattended-kit suite no unit of this build scopes, to declare a suite the hook cannot deny;
  (c) breaks the sibling shape F8 chose. RESOLVED (agent, 2026-09-16, delegated): (a); the extensions
  units 12 and 34 add keep the ceiling there, and a run that cannot fit is a finding at the
  post-build bar, never a quiet raise. That suite is an unattended-kit self-test, off the bar and run
  only on demand through `tools/unattended/run-unattended-gates.sh`, so in this build AC10's direct
  read of the leg's `ceiling` is what binds the bound.
...
- rev-4 · 2026-09-16 · regrounded on fb07ca25 (origin/main). §4's measured figures re-derived at the
  new base, where the landed builds filed about sixty asks; §5 perf and §10 follow. S11 and AC10 bound
  the leg's ceiling by the `DIRECT_CHECK_BOUND` aDeferredBar's gate-guard suite grades, with new §8
  F9, and hands-offs 12 and 34 carry the bound; F9 and AC10 say the gate-guard suite is an on-demand
  unattended-kit self-test, so AC10's direct read binds. AC11 gains a permission line, because
  aProbedUnit's child prompt forbids the hand-run hygiene gate its cost line had the pass run. Every
  §7 leg resolves and the four `[[gate_leg]]` siblings F8 copies are unchanged.
  ...
  the pass's direct check, which is what §8 F9 (a) bought by holding the ceiling at the bound, and
  only the two gate-leg commands beside it are deferred. AC11 keeps its line unchanged, its
  hygiene run being over this tree. Also checked and unchanged: no criterion of this unit asserts
  a phrase counts zero, the `New arm:` line names the selftest's own floor constant rather than
  `none`, and this unit adds text to no capped carrier.
  ...
  This leg's ceiling is held at that bound by §8 F9, so AC10's narrowed line stands and AC11 keeps
  its deferral, its hygiene run being over this tree. No criterion of this unit moves. The
  orchestrator also SET the `VERIFYING` run: `GATE_FULL=1 GATE_SELFTESTS=1 bash tools/run-gates/run-gates.sh`,
  plus the suite runs it attributes beside the bar. AC10's cost line now spells it that way, because
  the leg this unit adds is HELD like its siblings (§8 F8) and a plain bar would skip it while
  reading green; AC11's deferral is to the unheld `memory hygiene` leg, which any bar runs.
  Close-out verification rewrote that cost line, which had said the selftest RUN binds at
  `VERIFYING` while the permission line beside it said the pass makes it — two answers to one
  question inside one criterion. It now separates them: the flag run stays in the pass because its
...
copies at `09a22d2b`; `TOOL-cSpliceWarden-4` and `-7` since reconciled the archives, and `abac6d59`
held 599 ids with 521 live and 78 archived copies. BASE holds 658 with 580 live and the same 78
archived. The design's 275 triage asks on 60 builds was 288 on 56 at `abac6d59` and is 326 on 59 at
BASE. The design put the adopter census and `--ingest` in this unit; the brief's label table moves
...
BASE is `fb07ca25`. Against `abac6d59` it leaves the generator, `tools/govkit/subject-pins.tsv` and
`tools/run-gates/run-selftests.sh` byte-identical. The hygiene engine moved only its kit version
constant, and `tools/memory-tree/kit.toml` still declares the four module-selftest siblings F8
copies. The same seven legacy BLOCKED and DEFERRED rows stand. Two landed mechanisms bind this unit.
aProbedUnit's child prompt in `tools/workflows/unattended-unit.js` forbids any gate inside a pass, so
AC11's hygiene verdict belongs to the post-build bar. aDeferredBar's gate-guard suite, an on-demand
unattended-kit self-test and not a bar leg, reds a `--selftest` leg above `DIRECT_CHECK_BOUND` that it
does not declare, so §8 F9 bounds this leg and AC10 reads the bound directly.
