...
**Status:** SPECCED · rev-1 · 2026-09-14 · node d · Tier-2 · base abac6d59 · streams tooling · order 20
...
| [2026-09-14-review-TOOL-dDerivedDocket-1-spec-audit-g3-round1.md](../reviews/2026-09-14-review-TOOL-dDerivedDocket-1-spec-audit-g3-round1.md) | spec-audit | TOOL-dDerivedDocket-15 TOOL-dDerivedDocket-16 TOOL-dDerivedDocket-17 TOOL-dDerivedDocket-18 TOOL-dDerivedDocket-19 |
...
- **S1** One predicate in `tools/unattended/lib-unattended.sh` names every path in `SHARED_RECORDS`
  ...
  reports it under a new check code. It reads no memory-tree state. Observed by AC1.
...
- **S3** Protocol text, in `tools/unattended/PROTOCOL.template.md` and its installed copy, true in
  ...
  build's `BACKLOG.md`; and one pointer names the companion guide. Net growth at most 300 B.
  Observed by AC3.
- **S4** A companion guide: `tools/unattended/ASKS.template.md`, installed as
  ...
  descriptor with an LF pin, and byte-compared as a third pair in leg check 10 beside the protocol
  and verbs pairs. It carries the ask contract of design §19.2 to §19.6 as the rulings amended it,
  and the `asks-disposed` terms, override route and KEEP rule unit 17 hands off. Observed by AC4.
...
- **S6** Skill steps, each pointing into the guide for its rule: orientation per mandated ask,
  ...
  same commit, the repoint rule, the authority rule unit 19 states in the protocol, and a pre-flip
...
- **S7** The build method, in `tools/memory-tree/BUILD-METHOD.template.md` and its rendered copy: one
  ...
  disposed, derived from `gen_build_index.py --asks --build <slug>`. Net growth at most 250 B and 3
  lines. Observed by AC7.
...
- **S9** A sweep of every protocol and Skill sentence that describes ids, slugs, the prompt path or
  ...
  with its verdict in this unit's build record. Observed by AC9.
...
- **consumes-from** `TOOL-dDerivedDocket-15` — the clause grammar, READY and `--probe` that the guide
  states as contract and orientation calls.
...
| Carrier sentence at BASE | Rewritten as |
|---|---|
| M6 clause 3, "`memory/DECISIONS.md`, `memory/backlog/*.md`, the run-state file" | "`memory/DECISIONS.md`, an authored backlog shard, the run-state file": a generated view is already the clause's last case |
| protocol §2, "a planned unit is minted as a backlog row" | a planned unit is minted as an ask in the run's own build before the run-state file names it |
| protocol §11, fails 1 or 2, "a BACKLOG row naming what was seen" | an ask filed in the run's own build, carrying what was seen and why it was declined |
| `tools/unattended/lib-unattended.sh:202-205`, "template section 1 MANDATES a backlog row" | S2's sentence |

### The companion guide

Sections, in order: which invocation reaches which route (E1 and the E3 recipe; E2 dropped, E4 and
E5 routed to E3, design §20.1); the mandate's six properties with P5 and P6; orientation per ask,
including the stale, duplicate and REOPEN records of design §19.5 and §19.6; owner-call parking, the
four rows and the park; discovery filing; the `asks-disposed` terms with the F3 hardening, the
override route of ruling D12-b and the KEEP rule of ruling D12-c; the repoint rule. The grammar and
...
(ruling D12-h, method b), and the unattended suites run at the one post-build bar.
...
- error / empty / loading states — the refusal names each overlapping pair; blank keys compare as
  empty sets and pass.
...
- testing — arms for both new branches observed by hand; the harness arm in its own suite; the
  byte-compare legs and `kit/dogfood doc parity` at the post-build bar.
...
- **AC1** — When a scratch fixture's conf declares `memory/backlog` in `SHARED_RECORDS` and as the
  index half of a `GENERATED_INDEXES` pair, `bash tools/unattended/unattended.sh --status` refuses
  at conf load naming both keys, and `bash tools/unattended/check-unattended.sh` reports the new
  check; with gov's conf both are silent.
  Red when: the predicate compares strings for equality, so `memory` in one key and `memory/LIVE.md`
  in the other pass.
...
- **AC3** — When both copies of the protocol are compared, `bash tools/unattended/check-unattended.sh`
  ...
  300 B.
  Red when: an edit lands in one copy only, or the copy breaches the guide cap other units share.
...
- **AC4** — When `bash tools/unattended/adopt-unattended.sh --check` runs, it reports
  `UNATTENDED-ASKS.md` installed and equal to `ASKS.template.md`, and check 10 reports three pairs.
  Red when: the guide is installed with no byte-compare pair, so the adopter's copy drifts unseen.
...
- **AC9** — When this unit's sweep record under `memory/builds/dDerivedDocket/build/` is read, it
  ...
  with kept or changed and the driver behaviour it was read against.
...
New arm: `tools/unattended/check-unattended.test.sh` · a conf with one path in both keys · `ARMS_FLOORS` for `tools/unattended/check-unattended.sh`
