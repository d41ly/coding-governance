...
T3 would have signed 67 closures that the evidence contradicts in words. Of those, 46 were commit
bodies saying the ask stays open, 20 were commits about the ask's own same-id spec, and one was a
mined phrase about another shape. §8 F9 records the fork and §4 the measurement. At rev-6, CLOSED
comes only from a spec header's `closes` verb or a commit subject. So this worksheet signs no
closure, and each of the 67 KEEP rows names the rule that refused it.
...
with no ref. Its tree is `e21fa86f`'s with ten synthetic ask rows appended to
...
- AC1 — `--check` — two signing runs over the pair wrote byte-identical records, 31140 and 41503
...
- AC12 — amended rev-6 — `unit-inserted`: the criterion now uses the planner's option spelling and
  ...
  unsigned census reads 0.
- AC13 — `--tail switch` — the pair's triage `#` line named the fixture commit. Its one added row
  cites `memory/backlog/TOOL.md:533`, which holds `TOOL-aDeferredBar-13` in the working tree and a
  WITHDRAWN row for the added ask at the fixture commit. `--tail switch` then `--tail landing` wrote
  four records whose rows equal the default rows plus that one row, signed `T4 WONTDO` from the
  status slot read there. Each header names both worksheet paths and blobs and the fixture's sha.
...
- AC15 — `KEEP` — at the fixture commit, four rows each signed KEEP: a proposed WONTDO over an OPEN
  ...
  which is not live (T5), and a DEFERRED over an OPEN slot (T5). Two positive controls, a BLOCKED
  and a DEFERRED on the live `TOOL-dDerivedDocket-34` whose rows name it, signed `T5 BLOCKED` and
  `T5 DEFERRED`.
