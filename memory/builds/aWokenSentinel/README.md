...
| 5 | `TOOL-aWokenSentinel-5` | CLOSED | `resume-tick.sh`: the OS-scheduled out-of-process resumer, acting on a stale lease only |
| 6 | `TOOL-aWokenSentinel-6` | CLOSED | the contract: protocol §5, the Skill, the conf knobs and the kit version, with the cron job demoted to the idle-wake |
| 7 | `TOOL-aWokenSentinel-7` | CLOSED | `keepalive-reaped` becomes CHECKED at `--landed` against the cron listing the stop-guard recorded, no longer attested |
...
| 9 | `TOOL-aWokenSentinel-9` | CLOSED | the stop-guard listing as a FIELD on `--status`'s one line, omitted when unrecorded, and the header's one-line promise made an arm (audit H1) |
...
| 12 | `TOOL-aWokenSentinel-12` | CLOSED | the tick consults login BEFORE it kills: a logged-out node kills nothing, observed by an arm whose live process survives, and the class left-shifted to `memory/gotchas/` (audit H4) |
| 13 | `TOOL-aWokenSentinel-13` | CLOSED | the tick sources the root `.unattended.conf` into its own shell before its `read_bound_key` calls, so a declared bound is honoured and the NOTE names the file; arms under a declared key (audit H5) |
...
| 15 | `TOOL-aWokenSentinel-15` | CLOSED | the spec-audit commission's blob-pin pre-flight: the harness's resolver returns each subject's committed blob and working-tree hash, and refuses to dispatch a lens while any differ (audit round 2 B1, B2) |
...
| 17 | `TOOL-aWokenSentinel-17` | CLOSED | the driver suite reads `--status` by FIELD: `extract_next` armed on a suffixed line and `check_status_one_line` armed against a two-line driver copy (audit round 2 H2, H3) |
| 18 | `TOOL-aWokenSentinel-18` | CLOSED | `read_bound_key` refuses with exit 2 a caller that named no conf, so a bound read from a shell with `CONF` unset is a refusal rather than a default with an empty NOTE (audit round 2 H4) |
| 19 | `TOOL-aWokenSentinel-19` | CLOSED | `adopt-unattended.test.sh` declares a shrink-only `FLOOR_ASSERTIONS`, and the close's `run-unattended-gates.sh` run is the named observer of every arm the committed `seed()` feeds (audit round 2 H5) |
...
| 21 | `TOOL-aWokenSentinel-21` | CLOSED | the build harness's suite `unattended-build.test.sh` joins the declared self-test population: a held leg, a budget row, a registry exemption and a shrink-only `FLOOR_ASSERTIONS`, so its supplied-subject fixtures are an executed arm for unit 15's compare (audit round 3 H1) |
...
| 24 | `TOOL-aWokenSentinel-24` | CLOSED | the tick's conf-block staged break is ONE artifact: `build_tick_without_conf_block` in the tick's suite makes the copy from two anchor lines and asserts its shape, so spec 13's and spec 18's arms name a helper and no count (audit round 3 H4) |
...
ids TOOL-aWokenSentinel-26 TOOL-aWokenSentinel-27 TOOL-aWokenSentinel-28 TOOL-aWokenSentinel-29 TOOL-aWokenSentinel-30 TOOL-aWokenSentinel-31

<!-- gen:build-units -->
| Unit | Order | Tier | Status | Rev | Last change |
|---|---|---|---|---|---|
| [TOOL-aWokenSentinel-1 — the run-state file records the LEASE: `session:` and `pid:` at preflight, and `--resume --keepalive-id` replaces it](spec/2026-09-16-spec-TOOL-aWokenSentinel-1.md) | 1 | 2 | CLOSED | rev-5 | 2026-09-21 |
| [TOOL-aWokenSentinel-2 — `--liveness <slug>`, the one machine-readable predicate every out-of-session reader shares](spec/2026-09-16-spec-TOOL-aWokenSentinel-2.md) | 2 | 2 | CLOSED | rev-6 | 2026-09-21 |
...
| [TOOL-aWokenSentinel-8 — the stop-guard's `landing-unstamped` row: a bound session at `FINISHED-UNSTAMPED` is blocked and told to run `--landed`](spec/2026-09-20-spec-TOOL-aWokenSentinel-8.md) | 8 | 2 | CLOSED | rev-4 | 2026-09-21 |
...
| [TOOL-aWokenSentinel-5 — `resume-tick.sh`, the OS-scheduled out-of-process resumer](spec/2026-09-16-spec-TOOL-aWokenSentinel-5.md) | 11 | 2 | CLOSED | rev-6 | 2026-09-21 |
| [TOOL-aWokenSentinel-12 — the resume tick consults login BEFORE it kills: a logged-out node kills nothing](spec/2026-09-20-spec-TOOL-aWokenSentinel-12.md) | 12 | 2 | CLOSED | rev-4 | 2026-09-21 |
...
| [TOOL-aWokenSentinel-6 — the contract: protocol section 5, the Skill, the README, the conf prose and the dossier, with the cron job demoted to the idle-wake](spec/2026-09-16-spec-TOOL-aWokenSentinel-6.md) | 15 | 2 | CLOSED | rev-5 | 2026-09-21 |
...
| [TOOL-aWokenSentinel-7 — `keepalive-reaped` becomes CHECKED: `--landed` reads the harness's own cron listing](spec/2026-09-16-spec-TOOL-aWokenSentinel-7.md) | 17 | 2 | CLOSED | rev-6 | 2026-09-21 |
| [TOOL-aWokenSentinel-9 — the stop-guard listing as a FIELD on `--status`'s one line, and the one-line promise made an arm](spec/2026-09-20-spec-TOOL-aWokenSentinel-9.md) | 18 | 2 | CLOSED | rev-2 | 2026-09-21 |
| [TOOL-aWokenSentinel-19 — the adopter suite declares a shrink-only `FLOOR_ASSERTIONS`, and the close's kit-gate run is the named observer of every arm the committed `seed()` feeds](spec/2026-09-20-spec-TOOL-aWokenSentinel-19.md) | 19 | 2 | CLOSED | rev-2 | 2026-09-21 |
...
| [TOOL-aWokenSentinel-24 — the tick's conf-block staged break is ONE artifact: `build_tick_without_conf_block` in the tick's suite makes the copy from two anchor lines and asserts its shape, so both arms that stage it name a helper and no count](spec/2026-09-20-spec-TOOL-aWokenSentinel-24.md) | 24 | 2 | CLOSED | rev-2 | 2026-09-21 |
