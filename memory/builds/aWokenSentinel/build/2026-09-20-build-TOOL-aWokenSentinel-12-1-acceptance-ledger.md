...
- AC4 — amended rev-4 — the record carries one killers heading and names the arm gating the launched-pid instance (round 2, defects E and G). At the tip `grep -c '^## Where this repo.s killers live'` over `memory/gotchas/destructive-step-before-its-precondition.md` printed `1` (`2` at `8cc0b391`), and its `## Gating` region cut by awk carried `AC13` 1 (0 at `8cc0b391`); `python tools/memory-tree/gotchas.py --write` wrote `memory/gotchas/INDEX.md (75 record(s))` with the record's anchor count 5 -> 6 (`tools/unattended/lib-unattended.sh` joins), `--check` exited 0 afterwards and `--report` printed `unanchored : 0`. OBSERVED, 2026-09-21.
