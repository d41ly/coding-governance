...
- AC10 — the lease block alone over the sourced prologue read `n 54 -> 61, st=0` on node `a`, 2026-09-21: after AC1's preflight `sed -n 's/^host: //p'` printed `compeeto-agent`, the value `read_host_name` printed in the suite's shell (`COMPUTERNAME` lowercased), `grep -c '^pid-image: absent$'` printed `1` for pid 4242, and the `lease-utc:` line matched the UTC regex once; with `CLAUDE_PID` set to the suite shell's WINPID `pid-image:` read `bash.exe`, what `read_pid_image` printed for it; with `CLAUDE_PID` unset, `pid-image: absent`. RED against a driver copy without the three `set_fact` lines (five rows, `host recorded as this node: expected [compeeto-agent], got []` first). OBSERVED; the rev-5 fold of the closing review's ids 2 and 15.
