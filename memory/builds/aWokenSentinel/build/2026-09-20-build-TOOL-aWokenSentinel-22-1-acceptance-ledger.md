...
`spec tokens` and `install-prefix` legs, which run once at the close. No spec fold was owed: the
design's code ran as written on both drivers, and the executed count it adds is nine.
