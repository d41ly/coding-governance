...
**Status:** SPECCED · rev-1 · 2026-09-16 · node a · Tier-2 · base 5f9648d6 · streams tooling · order 3
...
- **S2** — the decision on a bound session, in this order: liveness verdict `TERMINAL` or
  `FINISHED-UNSTAMPED` allows; a non-empty `background_tasks` allows; a block count for this
  session at or above `STOP_GUARD_BLOCKS` allows; everything else blocks with the §4 reason. An
  unreadable liveness allows and says so. Observed by AC2, AC3, AC4, AC5, AC6, AC7.
...
- **S5** — the knob `STOP_GUARD_BLOCKS`, read by the hook from `.unattended.conf` with the kit
  default 12, announced on stderr when the project declares none, declared in the kit descriptor's
  `optional_keys` and in the shipped conf example with one comment line. Observed by AC6, AC7.
...
- **No prose carriers.** The kit README, the Skill template and its render, the protocol, the
  root `.unattended.conf` with its rationale comment and the map dossier are unit 6's, which
  restates every earlier unit's vocabulary in one commit. This unit's documentation is the hook's
  own header, which states what it does and does not check.
...
- **hands-off** `TOOL-aWokenSentinel-6` — every prose carrier named under §3, the root conf's
  `STOP_GUARD_BLOCKS` line with its rationale, the kickoff-manifest re-stamp that edit owes, and
  the version step.
...
stdin JSON -> hook_event_name is "Stop" and no agent_id  -> else exit 0, nothing written
  ...
  -> readBoundKey(root, STOP_GUARD_BLOCKS, 12)
...
| `FINISHED-UNSTAMPED` | any | any | allow | `finished-unstamped` |
...
an `export` prefix admitted — and applies the driver's rule: absent takes the kit default 12 and
prints `unattended: NOTE - this project declares no STOP_GUARD_BLOCKS, so a bound session is
blocked at most 12 times per run and session. Declare one in .unattended.conf to change it.` on
stderr; a
declared value that is not a positive integer takes the `knob-malformed` row. Twelve is three
keepalive cadences at the repo's ten-minute interval plus headroom for a run that legitimately
stops and continues several times inside one phase; it is PINNED here and re-declared with a
reason where a project wants another. The key joins `optional_keys` in `tools/unattended/kit.toml`
beside `UNIT_STALL_BOUND` and the shipped `.unattended.conf.example` with one comment line. This
repo's root `.unattended.conf` is NOT edited here: it is a kickoff-manifest watch path, and unit 6
adds all four knobs with their rationale in one commit and re-stamps the manifest once (§8 F3).
...
`unattended.sh` that prints the file `$STOP_GUARD_TEST_LIVENESS` names on `--liveness` and exits
with `$STOP_GUARD_TEST_LIVENESS_RC`; `FIX` is a scratch tree holding `.git/HEAD`, a
`.unattended.conf` and one `memory/builds/fx/RUN.md` with a `## Run facts` region; `P` is the
payload built by python, never hand-spliced, with `cwd` resolved through `cygpath -m`. The stub is
what makes the predicate observable in milliseconds without the driver's startup, and it is what
the suite runs. ONE integration arm uses a real `git init` fixture seeded the way
`adopt-unattended.test.sh`'s `seed()` builds one and the REAL driver copied beside the hook, so
the spawn, the `cwd` and the `verdict:` parse are exercised once against unit 2's verb. The suite
prints `PASS (<n> assertions)` against a derived `FLOOR_ASSERTIONS`, the shape of
`gate-guard.test.sh`, and every arm is observed RED first on the break it names.
...
`main`. Constants `KIT_STOP_GUARD_VERSION`, `BLOCKS_DEFAULT`, `LIVENESS_BOUND_MS`, `REASONS`. All
...
| `tools/unattended/kit.toml` | `stop-guard.test.sh` joins the `project-owned` include list; `STOP_GUARD_BLOCKS` joins `optional_keys` |
| `tools/unattended/.unattended.conf.example` | `STOP_GUARD_BLOCKS="12"` with one comment line, beside `UNIT_STALL_BOUND` |
...
- **AC2** — When `FIX`'s record carries `session:` equal to `P`'s and the stub's liveness file
  ...
  with `python -c 'import json,sys;…'`, never by grep over the raw bytes.
  Red when: the hook exits 0 with empty stdout on an open run, or the line is not written.
- **AC3** — When the liveness file holds `verdict: TERMINAL`, and separately `verdict:
  FINISHED-UNSTAMPED`, the invocation prints `rc=0` with empty stdout and the sidecar's new line
  carries `decision` `allow` and `reason` `terminal`, then `finished-unstamped`.
...
- **AC6** — When `FIX`'s conf declares no `STOP_GUARD_BLOCKS`, stderr carries `declares no
  STOP_GUARD_BLOCKS` exactly once and the block reason reads `1/12`; when it declares
  `STOP_GUARD_BLOCKS="zero"`, the invocation allows with `reason` `knob-malformed` and `rc=0`.
  figure: 12 is PINNED in the hook as `BLOCKS_DEFAULT`; the criterion reads it from the reason.
...
- **AC11** — When the real driver is copied beside the hook in a `git init` fixture whose record
  ...
  fixture: a `git init` tree seeded like the adopter suite's, built by the pass under the run's
  scratchpad; the driver needs unit 2's verb, which is landed by order.
...
New arm: tools/unattended/stop-guard.test.sh · a fixture record bound to the payload's session against a stub driver, one arm per §6 payload, one arm against the real driver on a git fixture; the break is the hook absent, then each row of the §4 table negated · `FLOOR_ASSERTIONS` derived at the suite's first green, none moved
