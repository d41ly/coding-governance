# Build brief — TOOL-aWokenSentinel-3

**Serves:** journal TOOL-aWokenSentinel-3

The pass this brief was handed to builds unit 3 of `aWokenSentinel`: `stop-guard`, the `Stop` hook. The spec-audit round
...
The spec is `memory/builds/aWokenSentinel/spec/2026-09-16-spec-TOOL-aWokenSentinel-3.md` and it is authoritative. You add `tools/unattended/stop-guard.js`, `tools/unattended/stop-guard.fragment.json`, `tools/unattended/stop-guard.test.sh` and the shared `tools/unattended/run-lease.js` the spec names; you generalise the wiring arm in `tools/unattended/adopt-unattended.sh` to every fragment the kit ships; you wire the fragment into `.claude/settings.json` with `python tools/settings-merge.py --fragment tools/unattended/stop-guard.fragment.json` (and extend `tools/settings-merge.py` and `tools/check-wiring.sh`'s fragment reader ONLY if they refuse an event with no matcher — measure first); the `STOP_GUARD_BLOCKS` knob read from `.unattended.conf`; the `kit.toml` rows. Unit 4 requires `run-lease.js` after you: give it the two exports the spec names and nothing else. Watched file: `.unattended.conf` — stamp.
...
- **The acceptance ledger** is a tracked record at
  `memory/builds/aWokenSentinel/build/2026-09-16-build-<id>-1-acceptance-ledger.md`
