# Build brief — TOOL-aWokenSentinel-6

**Serves:** journal TOOL-aWokenSentinel-6

The pass this brief was handed to builds unit 6 of `aWokenSentinel`: the contract. The spec-audit round
...
The spec is `memory/builds/aWokenSentinel/spec/2026-09-16-spec-TOOL-aWokenSentinel-6.md` and it is authoritative. You rewrite section 5 of `tools/unattended/PROTOCOL.template.md`, the two keepalive sections of `tools/unattended/SKILL.template.md`, the VERBS template where the spec says, `tools/unattended/README.md`, the knob prose in `.unattended.conf` and `tools/unattended/.unattended.conf.example`, and the dossier `memory/map/features/unattended.md` (mind its byte cap — trade bytes); you re-render every template with `bash tools/unattended/adopt-unattended.sh` in the SAME commit. Run the spec's two greps over every carrier and put their counts in the commit message. Watched file: `.unattended.conf` — stamp.
