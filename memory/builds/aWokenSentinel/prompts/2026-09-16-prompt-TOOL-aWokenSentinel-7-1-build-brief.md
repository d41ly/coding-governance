# Build brief — TOOL-aWokenSentinel-7

**Serves:** journal TOOL-aWokenSentinel-7

The pass this brief was handed to builds unit 7 of `aWokenSentinel`: `keepalive-reaped` becomes CHECKED. The spec-audit round
...
The spec is `memory/builds/aWokenSentinel/spec/2026-09-16-spec-TOOL-aWokenSentinel-7.md` and it is authoritative. You add to `tools/unattended/unattended.sh` the `--landed` refusal that reads the last line of the stop sidecar, the `unchecked` line, the `--status` line and the `--close` row wording; the arms in `tools/unattended/unattended.test.sh`; the VERBS entries with render; you CLOSE backlog row `TOOL-aPromptedMandate-11` in `memory/backlog/TOOL.md` in this commit, citing the evidence. Do not edit `stop-guard.js`; if the sidecar's shape does not carry what you need, bump unit 3's spec, never the hook, and return `committed:false` with the reason.
