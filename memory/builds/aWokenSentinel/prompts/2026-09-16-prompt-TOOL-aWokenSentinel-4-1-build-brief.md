# Build brief — TOOL-aWokenSentinel-4

**Serves:** journal TOOL-aWokenSentinel-4

The pass this brief was handed to builds unit 4 of `aWokenSentinel`: `stall-recorder`, the `StopFailure` hook. The spec-audit round
...
The spec is `memory/builds/aWokenSentinel/spec/2026-09-16-spec-TOOL-aWokenSentinel-4.md` and it is authoritative. You add `tools/unattended/stall-recorder.js`, `tools/unattended/stall-recorder.fragment.json` and `tools/unattended/stall-recorder.test.sh`, requiring `run-lease.js` by `__dirname`; you wire the fragment into `.claude/settings.json` through `tools/settings-merge.py --fragment`; you add the `kit.toml` rows. The adopter's generalised fragment loop from unit 3 asserts it wired — add no second arm. No conf key, no manifest stamp.
