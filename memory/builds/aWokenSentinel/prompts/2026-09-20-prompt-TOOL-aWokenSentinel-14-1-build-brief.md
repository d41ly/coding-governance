# Build brief — TOOL-aWokenSentinel-14

**Serves:** journal TOOL-aWokenSentinel-14

The pass this brief was handed to builds unit 14 of `aWokenSentinel`: `seed()` commits once, so every fixture that borrows it has a born HEAD. The spec-audit round
...
The spec is `memory/builds/aWokenSentinel/spec/2026-09-20-spec-TOOL-aWokenSentinel-14.md` and it is authoritative. This unit was PROMOTED by a spec-audit disposal to close confirmed findings; the review record its spec cites is the defect statement. You build exactly what section 4 designs and section 2 scopes; the files it names in section 2 are: `borrowed-seed-inherits-its-head-state.md`, `memory/gotchas/INDEX.md`, `run-unattended-gates.sh`, `tools/unattended/adopt-unattended.test.sh`. A template edit re-renders in the same commit; a new `fail` or `check` number is re-derived from the tree, never copied from the spec; a new gate leg also needs its `tools/gate-legs.json` row, its `PASS` line, its dossier claim and its subject pin where the spec says so.
