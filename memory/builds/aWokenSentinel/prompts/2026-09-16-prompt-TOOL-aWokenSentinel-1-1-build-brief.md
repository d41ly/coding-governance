# Build brief — TOOL-aWokenSentinel-1

**Serves:** journal TOOL-aWokenSentinel-1

The pass this brief was handed to builds unit 1 of `aWokenSentinel`: the run-state file records the LEASE. The spec-audit round
...
The spec is `memory/builds/aWokenSentinel/spec/2026-09-16-spec-TOOL-aWokenSentinel-1.md` and it is authoritative. You add to `tools/unattended/unattended.sh`: the two `set_fact` calls for `session` and `pid` in `verb_preflight` right after the keepalive is recorded, the `absent` fallback with its ONE stderr NOTE, the `--keepalive-id` option on `--resume` (extend the existing option parser, never a second one) re-recording `keepalive`, `session` and `pid` on a non-terminal record and refusing through `refuse_if_terminal` on a terminal one, the scaffold header comment; the arms in `tools/unattended/unattended.test.sh`; the `--preflight` and `--resume` entries in `tools/unattended/VERBS.template.md` and section 2's fact list in `tools/unattended/PROTOCOL.template.md` with BOTH renders under `memory/guides/`. Grep every carrier for the "twelve facts" count before you commit. No conf key, no manifest stamp.
