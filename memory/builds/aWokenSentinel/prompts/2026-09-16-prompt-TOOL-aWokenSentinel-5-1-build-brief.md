# Build brief — TOOL-aWokenSentinel-5

**Serves:** journal TOOL-aWokenSentinel-5

The pass this brief was handed to builds unit 5 of `aWokenSentinel`: `resume-tick.sh`, the out-of-process resumer. The spec-audit round
...
The spec is `memory/builds/aWokenSentinel/spec/2026-09-16-spec-TOOL-aWokenSentinel-5.md` and it is authoritative. You add `tools/unattended/resume-tick.sh` and `tools/unattended/resume-tick.test.sh` with the stub `claude` the spec describes; the `RESUME_ATTEMPTS` and `RESUME_TURNS` knobs; the registration section in `tools/unattended/README.md`; the `--status` line; the adopter `--check` INFO line; the `kit.toml` rows; `.unattended.conf` and the example. If the spec names `tools/unattended/lib-unattended.sh` or `.lexicon.conf`, read why before touching either. NO arm runs the real `claude`. Watched file: `.unattended.conf` — stamp.
