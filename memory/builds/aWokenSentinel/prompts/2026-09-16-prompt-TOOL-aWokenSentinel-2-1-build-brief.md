# Build brief — TOOL-aWokenSentinel-2

**Serves:** journal TOOL-aWokenSentinel-2

The pass this brief was handed to builds unit 2 of `aWokenSentinel`: `--liveness <slug>`, the one predicate. The spec-audit round
...
The spec is `memory/builds/aWokenSentinel/spec/2026-09-16-spec-TOOL-aWokenSentinel-2.md` and it is authoritative. You add the verb to `tools/unattended/unattended.sh` (VERBS_SLUG, the header invocation line the usage is derived from, the dispatch case, `verb_liveness`, and the shared last-move function EXTRACTED from `verb_audit` so both verbs call one implementation), the `RESUME_STALE_BOUND` knob through `read_bound_key`, the arms in `tools/unattended/unattended.test.sh`, the VERBS entry with its render, the protocol's key table with its render, the Skill's one invocation with its render, `tools/unattended/kit.toml` optional_keys, `.unattended.conf` and `tools/unattended/.unattended.conf.example` with the key's one-line rationale. Watched file: `.unattended.conf` — stamp `memory/guides/SESSION-KICKOFF.md`.
