# Build brief — TOOL-aProbedUnit-5

**Serves:** journal TOOL-aProbedUnit-5

The pass this brief was handed to builds unit 5 of `aProbedUnit`: scratch-guard denies an empty temp variable, `/tmp`, and a new entry at the POSIX root. The two spec-audit
...
The spec is `memory/builds/aProbedUnit/spec/2026-09-14-spec-TOOL-aProbedUnit-5.md` at rev-3 and it
is authoritative. Three predicates in `tools/hooks/scratch-guard.js` (`empty-var`, `tmp`,
`posix-root`), the `<os.tmpdir()>/claude` allowed root, a temp variable whose comparable value is
exactly `/tmp` contributing no allowed root, the arms in `tools/hooks/scratch-guard.test.sh` run
through the in-node environment form the spec's section 4 gives (the MSYS runtime rewrites a
POSIX-shaped TEMP handed to native node.exe — measured; never `env -u`), and the README's three
rules with what they do not catch. Each denial observed RED by its single arm before it is wired.
The hook's version constant stays. This hook fires on YOUR OWN Bash calls in this session: a
command of yours that trips a new rule is the rule working, not a bug — spell the path under the
scratchpad and go on.
