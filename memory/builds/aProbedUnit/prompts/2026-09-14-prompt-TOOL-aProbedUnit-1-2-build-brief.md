# Build brief — TOOL-aProbedUnit-1

**Serves:** journal TOOL-aProbedUnit-1

The pass this brief was handed to builds unit 1 of `aProbedUnit`: no gate, suite or bar runs inside a unit pass. The two spec-audit
...
The spec is `memory/builds/aProbedUnit/spec/2026-09-14-spec-TOOL-aProbedUnit-1.md` at rev-3 and it
is authoritative. You edit the child prompt in `tools/workflows/unattended-unit.js`, the parent's
`GROUND` in `tools/workflows/unattended-build.template.js` AND its render `unattended-build.js`, M6 of
`tools/memory-tree/BUILD-METHOD.template.md` AND its render `memory/guides/BUILD-METHOD.md` (byte
cap 27648, line cap 350 — the file is at 26743; the edit trades bytes), the Skill bullet in
`tools/unattended/SKILL.template.md` AND its render, and the harness suite arm the spec names.
Ordering fact: unit 2 edits the same child prompt after you; keep your paragraph a single
identifiable block so its paragraph lands beside it. Watched file: BUILD-METHOD.md — stamp.
