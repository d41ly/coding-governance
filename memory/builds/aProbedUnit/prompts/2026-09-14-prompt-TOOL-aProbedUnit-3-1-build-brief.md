# Build brief — TOOL-aProbedUnit-3

**Serves:** journal TOOL-aProbedUnit-3

The pass this brief was handed to builds unit 3 of `aProbedUnit`: `--audit <slug>`, the dispatched-unit stall probe. The two spec-audit
...
The spec is `memory/builds/aProbedUnit/spec/2026-09-14-spec-TOOL-aProbedUnit-3.md` at rev-3 and it
is authoritative. You add the verb to `tools/unattended/unattended.sh` (VERBS_SLUG, the header
invocation line the usage is derived from, the dispatch case, `verb_audit`, and the helpers the spec
names: `read_bound_key <NAME> <DEFAULT> <UNIT> <NOTE>` hoisted from the GATE_BOUND block with
GATE_BOUND and UNIT_STALL_BOUND routed through it, `check_pass_open`, `scan_dirty_paths`), the arms
in `tools/unattended/unattended.test.sh` (mkconf's SIXTH positional `UNIT_STALL_BOUND="${6-1800}"`,
the NOCONF hit), the VERBS and SKILL templates AND renders (the keepalive prompt runs `--audit`),
PROTOCOL section 8, `tools/unattended/kit.toml` optional_keys, `.unattended.conf` (declare
`UNIT_STALL_BOUND=1800` with its reason), the conf example if tracked, and the dossier
`memory/map/features/unattended.md` (20470 of a 20480-byte cap — trade bytes). Unit 6 lands after
you and routes `REVIEW_ROUNDS` through your helper as its third caller and takes mkconf's SEVENTH
positional: do not pre-add either. Watched file: `.unattended.conf` — stamp. The process-monitor
pointer line prints only where `tools/process-monitor/census.py` is tracked (it is, here).
