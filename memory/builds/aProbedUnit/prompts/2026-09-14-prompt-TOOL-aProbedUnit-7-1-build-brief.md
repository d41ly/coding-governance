# Build brief — TOOL-aProbedUnit-7

**Serves:** journal TOOL-aProbedUnit-7

The pass this brief was handed to builds unit 7 of `aProbedUnit`: disposal by severity, on any confirmed finding. The two spec-audit
...
The spec is `memory/builds/aProbedUnit/spec/2026-09-14-spec-TOOL-aProbedUnit-7.md` at rev-3 and it
is authoritative. The DISPOSAL stage in `tools/workflows/unattended-build.template.js` AND its render
runs whenever `auRaw.confirmed > 0` (announced on CONVERGED too), its prompt promotes every confirmed
BLOCKER and HIGH through `--rescope aProbedUnit --act add --item <id> --reason <text>` plus a spec at
its tier whose mechanism CLOSES the finding, folds every MEDIUM and LOW as a rev-N bump, and returns
`promoted`/`folded` counts on every non-throwing exit past the stage; `meta.phases[2].detail`
follows. M4's disposal sentence and line 143's "the exit's own disposition" in
`BUILD-METHOD.template.md` AND its render become the severity rule (byte cap — unit 1 and unit 6 have
already moved the file; measure against HEAD, not the base figure); the driver's `review_exit_note`
sentences and the check-37 clause with its comment take severity wording, keeping the substrings
the standing arms read; the Skill's NON-CONVERGENT and CONVERGED bullets and the VERBS pointer say
it once. The backlog flip of `TOOL-aProvenReuse-3` is the closing pass's, not yours (shared record).
Watched file: BUILD-METHOD.md — stamp.
