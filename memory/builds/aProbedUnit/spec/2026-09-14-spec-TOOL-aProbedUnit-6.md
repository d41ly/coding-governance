...
**Status:** CLOSED · rev-7 · 2026-09-14 · node a · Tier-2 · base 1b000d1a · streams tooling · order 6
...
| [2026-09-14-review-TOOL-aProbedUnit-1-diff-review-round2.md](../reviews/2026-09-14-review-TOOL-aProbedUnit-1-diff-review-round2.md) | diff-review | TOOL-aProbedUnit-1 TOOL-aProbedUnit-2 TOOL-aProbedUnit-3 TOOL-aProbedUnit-4 TOOL-aProbedUnit-5 TOOL-aProbedUnit-7 |
| [2026-09-14-review-TOOL-aProbedUnit-1-diff-review-round3.md](../reviews/2026-09-14-review-TOOL-aProbedUnit-1-diff-review-round3.md) | diff-review | TOOL-aProbedUnit-1 TOOL-aProbedUnit-2 TOOL-aProbedUnit-3 TOOL-aProbedUnit-4 TOOL-aProbedUnit-5 TOOL-aProbedUnit-7 |
...
- The state gate at `:4104` to `:4116`: the first case becomes `NON-CONVERGENT|CEILING|BOUNDED)`
  ...
  `fold` beside a non-zero blocker count in a record first-committed on or after the driver's
  `FOLD_CUTOFF` (rev-7: a record before it was written while the driver accepted the row, and is
  read as that contract read it). Nothing here
...
- rev-7 · 2026-09-14 · §4 (the state gate paragraph) · folded the closing diff review round 2, cluster A (id 6, the BLOCKER): the fold-beside-blockers clause graded every record first-committed on or after `DISPOSITION_CUTOFF`, which dates the FIELD and not the later rule about its value, so sixteen tracked append-only records the driver wrote while `fold` was legal at every terminal exit redded the bar. The driver declares `FOLD_CUTOFF="2026-09-14"` beside `REVIEW_DISPOSITIONS`, the leg reads it through `core_of`, derives `rv_foldgraded` from the same first-commit date by the same `sort -C`, passes it as `-v foldgraded=`, and accumulates `foldbad` only when it is 1; the message names the cutoff and no longer asserts the row was hand-written; an unreadable cutoff is named inside check 2's own failure rather than at a new `fail` site, because the pinned check-2 ordinals in `memory/project/unarmed-branches.txt` sit below the read. Arms in `check-unattended.test.sh`: `mkdisp` takes `DISPDATE`, the existing NON-CONVERGENT fold arm commits at the cutoff, a `blockers 2 · BOUNDED · disposition fold` record at 2026-09-01 prints no check 2 line and the same record at the cutoff reds, and a `mutate` unquoting the constant hits the liveness line — five arms run alone with the preamble sourced, all five red against a frozen copy of the base kit and silent at the tip. Leg suite floors +4, region one. The `fail 37` in the driver is unchanged.
