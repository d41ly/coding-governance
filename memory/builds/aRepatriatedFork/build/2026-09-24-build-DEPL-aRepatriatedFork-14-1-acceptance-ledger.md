...
- AC1 — `govkit check` — at `%TEMP%/a14i` no line reports `pytest-ini-knobs`, and the kit prints `landed-unmeasured`. The fixture arm `[aRF-14 AC1]` discharges the hole from `services/api/pyproject.toml` beside a root file with no pytest table, and it reds under a7c78ad2's root-only probe
