# Fold brief B — closing review round 1, M2, M3, M4 and residual e
...
Items **M2**, **M3**, **M4** and residual **(e)** — govkit's handling of role moves and of the
`[[own]]` declaration. Code: `tools/govkit/govkit.py`, `tools/govkit/selftest.py`. Specs:
`DEPL-aRepatriatedFork-17` (M2), `DEPL-aRepatriatedFork-13` (M3, M4), `DEPL-aRepatriatedFork-21`
(residual e).

- M2: the `--accept-role-moves` branch is taken only when the new role's disposition does not write;
  a move into `rendered` falls through to the existing reconcile or refuse path.
- M3: an `own.path` that is not already its own `posixpath.normpath` is refused by name.
- M4: `update` and `check` read the declaration through `resolve_owned_rows` before dispatch; an
  `engine` receipt row whose path is a declared owned destination is refused with
  `adopt --re-adopt` as the remedy, never written.
- (e): one row builder for an `adopter-owned` receipt row, called by both `adopt` and `apply`.
