...
- AC13 — `tFixture-169` — the membership test, which is what keeps the joins from answering the wrong question. An edge naming a spec the cutoff grandfathered is silent, and so is one naming a Tier-1 sibling: both are `next`-ed or never registered, so `pop` does not hold them and the join skips. Read honestly — the without-the-test comparison the criterion asks for was not staged; the arms were written WITH the membership test and its two silences observed, rather than the test being removed to watch them red.
