...
units-at-landing: TOOL-aJoinedCanon-1 TOOL-aJoinedCanon-2 TOOL-aJoinedCanon-3 TOOL-aJoinedCanon-4 TOOL-aJoinedCanon-5 TOOL-aJoinedCanon-6 TOOL-aJoinedCanon-7 TOOL-aJoinedCanon-8 TOOL-aJoinedCanon-9 TOOL-aJoinedCanon-10 TOOL-aJoinedCanon-11
unpushed-at-landing: 0
parked-surfaced: yes, 3 surfaced
keepalive-reaped: yes
witness: 68dfda8fdc2306d6ee9a430961b506f806c26aa1
phase: LANDED
mode: slug
...
2026-09-06T22:14:44Z review · item aJoinedCanon · reason verdict CLEAN WITH FIXES · blockers 0 · CONVERGED

2026-09-07T00:02:53Z decision · item memory-hygiene self-test now TIMES OUT at its 900 s ceiling under the full concurrent bar, because this build grew it. Raise the ceiling, or make the suite cheaper? · reason Measured, not estimated. The suite went from 99 assertion points to 121 (300 to 369 assertions) because this build added six arm groups and roughly thirty fixtures — the coverage it was commissioned to add. Standalone it still PASSES: 369 assertions, exit 0, in about 11 minutes against a 900 s ceiling. Under GATE_FULL=1 GATE_SELFTESTS=1 at width 8 it exceeded 900 s and the runner killed it, which is the cost-is-a-verdict rule working exactly as written rather than a defect in the suite. The one historical ledger row for this leg is 658 s ok, at the pre-build size; this repo has measured contention multiplying a hygiene leg roughly 2.9x elsewhere (48 s quiet, 139 s inside a full bar at width 8), so the ceiling was probably already unsafe under full concurrency before this build and was never observed because GATE_SELFTESTS is on-demand only and no boundary sets it. TWO OPTIONS AND I REFUSE BOTH. Raising the ceiling is the charter's own sanctioned remedy for slowness that fails rather than annoys, but a run raising a bound that its OWN additions breached has no external check on the judgement, and that is the one shape a cost gate must not have. Making the suite cheaper is real unspecced work on a kit this build was not commissioned to optimise, and the cost is process creation per the recorded gotcha, so it is a restructure and not a tweak. NOTE WHAT THIS DOES NOT BLOCK: the bar that gates the push is GREEN at 42 of 42, this leg is chunk=selftests which no boundary runs, and the other two self-test failures on that run — govkit selftest and codebase-map adopter e2e — were both verified to fail IDENTICALLY at BASE and are pre-existing, so the owner is not chasing three regressions here, only this one.
