...
**Status:** OPEN · rev-2 · 2026-08-31 · node a · Tier-2 · base 3bfc5e87 · streams tooling · order 1 · ratified 2026-08-31
...
- **S1** — a new cutoff key `SPEC10_EVIDENCE_CUTOFF` declared in `.memory-tree.conf` and mirrored
  BLANK in `tools/memory-tree/.memory-tree.conf.example`. Its semantics are `STREAMS_CUTOFF`'s, not
...
- **S5** — `tools/memory-tree/SPEC-TEMPLATE.template.md` §10 states the two required facts and names
  ...
  pair.
- **S5a** — the same template/render rule for `memory/guides/BUILD-METHOD.md`, whose source is
  `tools/memory-tree/BUILD-METHOD.template.md`, if M5's wording needs to name the new requirement.
...
| `tools/memory-tree/check-memory-hygiene.sh` | S1 the `-v ecut=` binding; S2–S4 the assertion |
...
- **AC5** — with `SPEC10_EVIDENCE_CUTOFF` temporarily set to `2026-08-04`, the same command reds and
  names **188** files. This is the liveness assertion for AC4: a predicate that reds nothing at its
  declared cutoff must be shown capable of redding. **188, not 253** — the figure the build README
  quotes is the all-tiers §10-bearing population, while S2's arms are reached only for the 264
  Tier-2 specs that survive the `Tier-1` cut. Re-derive both numbers at build time rather than
  copying either; a criterion pinned to the wrong population fails a correct implementation.
