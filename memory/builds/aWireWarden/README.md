---
slug: aWireWarden
node: a
opened: 2026-07-15
streams: tooling
roster: TOOL
ids: TOOL-aWireWarden-1
---

# aWireWarden — check-wiring.sh + the SessionStart hook that self-heals an unset core.hooksPath

Node `a` · opened 2026-07-15 · streams tooling.

Records live under `spec/`, `build/`, `reviews/` and `prompts/`. The table below is
GENERATED from the status header of every spec in this folder — do not hand-edit it.

<!-- gen:build-index -->
**Build status:** INPROGRESS · 1 unit(s) · node a · opened 2026-07-15 · streams tooling · ids TOOL-aWireWarden-1

| Unit | Status | Rev | Last change |
|---|---|---|---|
| [TOOL-aWireWarden-1 — Wiring-health check + SessionStart nudge for per-clone tool wiring](spec/2026-07-15-spec-aWireWarden-1.md) | INPROGRESS | rev-3 | 2026-07-15 |
<!-- /gen:build-index -->
