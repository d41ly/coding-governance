...
**Status:** CLOSED · rev-6 · 2026-08-25 · node a · Tier-2 · base 381008a1 · streams tooling · order 1 · ratified 2026-08-25
...
- **S14** — RETIRED BY MERGE, and the retirement is `TOOL-dSpentCeiling-1`'s, not this unit's.
  Read the rest of this item as history: main removed `READ_PATH_CEILING` and made check 16 structural
  while this build was in flight, so the mechanism the item moves no longer exists. It was true when
  written and is kept rather than deleted because the acceptance ledger cites it. AS WRITTEN: the
  read-path ceiling MOVES. This unit adds a section 8 row to
...
- rev-6 · 2026-08-26 · landed. S14 RETIRED BY MERGE: `TOOL-dSpentCeiling-1` removed the read-path
  byte budget on main while this build was in flight, so the ceiling S14 moved no longer exists and
  the movement was dropped whole in the merge resolution. AC13 amended to match. No other scope
  item was touched by the reconcile.
