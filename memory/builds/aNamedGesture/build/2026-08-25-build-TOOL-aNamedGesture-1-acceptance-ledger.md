...
- AC13 — amended rev-6 — the criterion is now UNMEASURABLE and that is not a shortfall: main
  retired the read-path byte budget (`TOOL-dSpentCeiling-1`) while this build was in flight, so
  `READ_PATH_CEILING` no longer exists and the movement it graded was dropped whole in the merge.
  What follows is the rev-5 record, kept because it is the honest history of the criterion.
  amended rev-5 — the criterion graded itself by re-typing a measurement, and the
