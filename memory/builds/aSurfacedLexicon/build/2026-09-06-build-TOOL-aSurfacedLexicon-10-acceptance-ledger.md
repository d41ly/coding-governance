...
- AC2 — the same fixture conf rewritten byte-wise to CRLF — the refusal STILL fires, and the arm asserts the `ALREADY EXPANDED` VERDICT rather than the exit code alone, because a non-zero for another reason satisfies the weaker form. A second arm covers the direction the CR residue actually inverts, an EMPTY `expanded=""`, and the platform half that cannot be exercised here is PROBED and announced.
...
- AC4 — the unruled tail on that same fixture — non-empty, DISJOINT from the proposal list, carrying the fixture's own off-canon tokens, under a `NOT PROPOSALS` header that says in words that these are not proposals and never will be, and that states the waived total so its rows reconcile with `--list`'s corpus-wide census.
