...
- **S1** — `SPEC10_CUTOFF` is declared with `SPEC_FORMAT_CUTOFF`, `STREAMS_CUTOFF` and
  `SPEC_WITNESS_CUTOFF`, above the conf source, keeping its present value `2026-08-04` as the
  shipped default.
...
- **S5** — an arm proving the conf value is read: a scratch tree declaring a cutoff that makes a
...
Satisfied for the set by `TOOL-aDeclaredBound-4` §10. This unit extends no seam and introduces no
mechanism: it moves one existing declaration from one part of a file to another part of the same
file, into a block of three keys that already do exactly what it will do. The reuse question worth
