...
2026-08-18T22:47:42Z override · item build-complete · reason TOOL-aDeclaredBound-4 is PARKED and unbuilt, and the run refused to start it rather than start it badly. It is the SECURITY-SHAPED unit: its eleven acceptance criteria cannot be observed until the Workflow fixture isolation exists, because every Workflow arm in the hook's harness ships no cwd today, so a hook that read a declaration would be tested by arms resolving against the repo root and passing by finding nothing. That is the exact class both spec audits and both closing-review rounds kept catching, and building it at the end of a long run would have shipped a fourth instance of it. Four of five units are CLOSED with the bar green at 72/72; what remains is enumerated in the spec at rev-4 and none of it is blocked. The parked region carries this, the eight unbuilt left-shift gates, and the note that unit 5's gate needed two remediation rounds with the two-round Tier-2 cap now spent.

2026-08-19T11:35:00Z abort · item aDeclaredBound · reason This run merged into local main and stopped at LANDING without marking itself; its merge IS reachable from main, but nothing was ever pushed, so the run landed nothing. LANDED is not available and would not be true — the driver's own check 15 refuses a LANDED claim whose witness is not an ancestor of the tip the remote advertises, and it refused this exact edit when it was attempted. ABORTED is the same terminal state this fleet's aFusedCharter run took for the same reason on the same day. Set by the aFusedCharter session on the owner's explicit instruction, as part of landing main; that session did NOT perform this run and attests nothing about its Definition of Done. The work itself is unaffected and reaches the remote with this push.
