...
2026-08-25T12:03:02Z rescope · item add TOOL-dPromptedSeam-4 · reason PROMOTED from the closing diff review's round-2 H1, under M4's non-convergence rule. The parity arm that TOOL-dPromptedSeam-3 added rides a subject:kit leg the runner holds unless GATE_SELFTESTS=1, and .githooks/pre-push sets GATE_FULL and not that -- so the equality is checked, and not checked where drift would land. Widening the leg's guard to name tools/lexicon made the causing edit SELECT the leg, which is necessary and not sufficient, because selection does not defeat the hold. Specced DEFERRED rather than built because its only open question resolves to a governance-carrier change: the smallest repair flips that leg to subject:repo and contradicts the standing 2026-08-23 owner ruling that a kit's self-tests are not merge-bar legs, which M3 veto 2 makes an owner turn. The alternative costs four declaration carriers for one frozenset comparison. The run's lean is to accept on-demand coverage and say so, which the DEAD_TOKENS comment now does.

2026-08-25T12:03:02Z decision · item where should the lexicon/map stopword parity assertion run — TOOL-dPromptedSeam-4 Q1 · reason Three options and the run may not pick, because the cheapest one changes a governance carrier. (a) Flip codebase-map kit selftest to subject:repo: smallest diff, but it puts a kit self-test on every bar and contradicts the 2026-08-23 owner ruling that took seven such legs off it -- M3 veto 2, an owner turn. (b) A dedicated repo-subject leg calling only the parity check: correct, cheap to run, and four declaration carriers to declare -- the leg row, a script, a gate_leg claim and the map dossier -- for one frozenset comparison, on the surface the govkit population gate grades. (c) Accept on-demand coverage: free, and arguably right, because the arm is a kit self-test by nature, its guard now names tools/lexicon so the causing edit selects it, and lexicon.py's DEAD_TOKENS comment now says 'gated on demand, not at the push boundary' rather than the flat 'GATED' that overclaimed. The run leans (c). What makes this an owner turn rather than a preference is that (a) is the only option whose cost is zero and it is the one that touches the ruling.
