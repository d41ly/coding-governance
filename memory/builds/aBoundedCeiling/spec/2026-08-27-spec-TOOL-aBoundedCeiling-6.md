...
Put a wall-clock bound on the merge-bar invocation `tools/unattended/unattended.sh` makes for the
`gates-green` Definition-of-Done item, so an unattended `--close` can no longer wait forever on a
gate that will never return. This is the only unit of this build that protects an adopter whose gate
...
- **N3** — the bound is not a per-leg ceiling and does not replace one. `TOOL-aBoundedCeiling-1`
  bounds each leg of gov's runner; this bounds the whole invocation of whatever runner a project
  declares. A project can have both, one, or neither.
...
- **observability** — a breach names the bound and the elapsed time in the DoD output the close
  already prints, and the defaulted case announces itself on stderr.
...
- **testing + left-shift gates** — AC1 through AC4 in `tools/unattended/unattended.test.sh`, with
  AC1 measuring ELAPSED time, because the message was always the correct half of this defect.
