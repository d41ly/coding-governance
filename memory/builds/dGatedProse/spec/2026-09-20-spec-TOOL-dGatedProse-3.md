...
that records the breach the raise removes, and record the reason where a reader of the constant finds
it. The owner ruled the raise on 2026-09-20 at `TOOL-dLoggedFlight-33` and named ONE key; the dry run
measured that moving that key alone is a raise in name only, because the line axis reds first. Moving
two where the row names one is a divergence this unit discloses rather than absorbs: the row's own
wording is corrected at landing, by the route section 3 names.

## 2. Scope (IN)

- **S1** — At `tools/memory-tree/check-memory-hygiene.sh:84`, both cap constants move together to the
  ratified figure, with a dated reason block immediately above them naming the ruling, the measured
  growth rate and the headroom the figure buys. Observed by AC1 and AC2.
...
**No kit-version move and no marker re-stamp.** The build ruled one move per build, owned by
`TOOL-dGatedProse-1`, which bumps the constant and re-stamps every carrier of the retired marker in
its own commit from a derived population. This unit moves no version string and edits no
`gov:kit memory-tree@` marker; what it relies on that unit having left is in the edge below, and
where the build's single move has to SIT is F2. Rev-1 owned both jobs and no longer does. This is the
one non-goal with a criterion of its own, because a sibling can break it and a green bar would not
say so: observed by AC3.
...
- **consumes-from** `TOOL-dGatedProse-1` — that unit lands first, edits the same engine file and the
  same self-test, and owns the build's single kit-version move. This unit reads four things it left,
  and asserts none of them. **The constant's value**, which S1 must not move again. **Every
  `gov:kit memory-tree@` carrier already re-stamped**, without which S2's render writes three live
  copies at a mixed marker. **The suite's assertion floor**, raised by the count of assertions that
  unit ADDS, which AC10 reads rather than pins. And **the line offsets**: that unit adds a preset to
  the engine's `*_CUTOFF` cluster, a blank declaration to the conf example above line 180, and a
  fixture block to the self-test, all of which sit ABOVE coordinates this spec cites. Every
  `path:line` here is a base-`fcbfba5f` coordinate and is re-resolved by grepping its anchor string
  before the edit. Nothing gates that: `tools/check-spec-tokens.py:396` resolves a citation against
  the file's line COUNT and reds at `:402` only when the number is out of range, so a citation that
  slid four lines passes, which is the class section 10 records five prior instances of.
- **consumes-from** `TOOL-dGatedProse-2` — that unit changes `memory/TEMPLATE-SPEC.md`, which is one
  of the four dogfood copies S2 regenerates. Its change must live in
  `tools/memory-tree/SPEC-TEMPLATE.template.md`, or S2's render deletes it. This is not advice: the
  render loop iterates the whole PAIRS table, so ANY unit that renders overwrites all four live
  copies from their templates, whichever one it edited.
- **hands-off** `TOOL-dGatedProse-4` — for the same reason, S2's render rewrites
  `memory/guides/BUILD-METHOD.md` from `tools/memory-tree/BUILD-METHOD.template.md`, so that unit's M4
  edit goes through the template. The render is byte-neutral for that pair, but not by this unit's
  doing: this unit pins no version string, so the neutrality is a property of the stamp
  `TOOL-dGatedProse-1` left, and it is what leaves that guide's headroom against its row in
  `tools/template-size-limits.txt` untouched.
- **consumes-from** external — the owner's ruling of 2026-09-20 and the dry-run measurement recorded
  in this build's research record. The ruling authorizes the raise and the row removal; this unit does
  not grant itself either.
- **hands-off** external — three writes this unit does not make: the adopter migration and the
  ...
  of `memory/backlog/TOOL.md:548` at landing.
...
This unit mints no identifier. Both keys already exist, no leg is added, no function is written. The
only new tracked text is one appended row in `memory/DECISIONS.md` and the dated comment block above
the constants.
...
shard, no verb. The two kit files it edits are already inside the dossier's own
...
### What the version move leaves, and what this unit relies on

The build's single kit-version move is `TOOL-dGatedProse-1`'s. This unit's stake in it is a
dependency, not a task, and it has two halves.

The first is the MARKER STATE. The carrier population is derived, never listed, by
`git grep -lF "gov:kit memory-tree@$OLD"`, and on this tree at base it returns nine files: the engine,
the four shipped `tools/memory-tree/*.template.md` templates, and the four rendered copies. Unit 1
re-stamps all of them in one commit, so this unit sees a UNIFORM marker rather than the mix rev-1's
own derivation would have found — rev-1 presupposed a single `$OLD` that, with the bump and the
re-stamp in different commits, no value would have satisfied.

The second is the RENDER DIRECTION, which is this unit's own work. `sed -n 58p
tools/memory-tree/kit-dogfood-parity.test.sh` prints the PAIRS table: which live copy renders from
which template. Its loop at `:100` iterates the whole table in every mode, so:
...
is why both hands-off edges bind, and it is the only way to know the four renders must be produced by
a command rather than typed. The script's header states the direction plainly, template to live, and
...
| `memory/backlog/TOOL.md:231` | OPEN row asserting the line bound; corrected in place |
...
| 73728 and 900, the conservative pair | inside the owner's bracket and buys 11458 B, which is 28.5 days; see the fork in section 8 |
...
route table marks "nothing to move", plus one appended decision row and the three live copies S2
rewrites without editing. Re-run the passes at build time; a count written beside a derived population
is wrong on the next commit, and rev-1's was made wrong by the ruling that moved the marker population
out of this unit.

## 5. Production-readiness checklist

- security — N/A. No write path, no input boundary and no authorization surface. The edit is two
...
- risks — four, and each is named where it bites. The build's single version move may sit at the wrong
  commit for `verdict epoch`, which is F2 and is not this unit's to place. S2's render rewrites three
  live copies this unit has no edit in, so a sibling that hand-edited a render loses it. The two
  self-test arms break inside a held suite, so a green bar does not clear them. Existing adopters are
  unreachable by either edit and stay red until they edit their own conf.
...
- **AC3** — When this unit's diff is read, it carries no change to `KIT_MEMORY_TREE_VERSION` and no
  `gov:kit memory-tree@` marker edit, and the constant reads the value `TOOL-dGatedProse-1` left.
  Red when: this unit bumps a second time, which the build reserves to unit 1 and which would make
  two moves per build out of one.
  NOT OBSERVED HERE: whether `verdict epoch (kit version dates the engine)` is green at the build's
  tip. That turns on where the single move SITS, which is F2 and is a build-level decision this unit
  cannot take in either direction.
- **AC4** — When `tools/check-kit-versions.sh` runs, it prints no line about a
  ...
  that checker print anything — that half belongs to `kit/dogfood doc parity`, and it is the half S2
  can actually break.
...
- **AC9** — When `tools/memory-tree/check-memory-hygiene.sh` runs over the guide fixture the suite
  ...
  never prints.
- **AC10** — When the diff is read, it adds no assertion to
  `tools/memory-tree/check-memory-hygiene.test.sh`, and the floor at
...
tokens. The seventh, kit/dogfood doc parity, is subject `repo` too but DOES carry a guard — six
paths, among them the live HYGIENE and TEMPLATE-SPEC copies, both rendered guides, `tools/lib/` and
`tools/memory-tree/` — and scope items S1, S2, S4 and S6 each touch one, so it arms on this unit's
bar either way. The eighth, the memory-hygiene self-test, is subject `kit` and chunk `selftests`,
guarded on `tools/lib/` and `tools/memory-tree/`, so it is held by default and owed only by the
kit-work Definition of Done. Rev-1 called the first four unguarded, which was wrong about the fourth,
and unit 4's cross-read was right. Leg names in this paragraph are deliberately unbackticked prose;
the graded list is the single line below, where each name is spelled as the manifest spells it.

`memory hygiene` · `verdict epoch (kit version dates the engine)` · `kit version markers` · `kit/dogfood doc parity` · `memory-hygiene self-test` · `testsuite counts (every bar self-test prints one)` · `codebase-map coverage + freshness` · `spec tokens (a spec's own names resolve)`
...
**F1 — the ratified figure. NOT this unit's to resolve.** The owner ruled the ACT on 2026-09-20 and
named no number. Two candidates survive, both inside the bracket the ruling implies, which is above
today's 62270 B and below twice the retired cap:
...
wants. **Recommendation: 81920 and 1000.** The intervention this replaces is the
`TOOL-dFoldedVerdict-5` split at `dc9431f8`, which amputated a whole section, recovered 7209 B and
lasted 20 days. A stopgap buying less than roughly twice that expires inside one build. This is left
open rather than taken because "roughly twice the split's lifetime" is a judgment about how long a
stopgap should last, not an arithmetic result — the skeptic said so in as many words, and 29 days
against 49 is exactly the size of the judgment. The resolver picks; every figure either pick needs is
in the table.

**F2 — FORK · where the build's single kit-version move has to SIT. NOT this unit's to take.** The
build ruled one move per build, owned by `TOOL-dGatedProse-1` at order 1, and this unit conforms: it
does not bump, and AC3 observes that. What the ruling leaves open is placement, and it must not be
discovered on a bar. Re-derived from source rather than from rev-1's own resolution:
...
scan set declared at `:68`, must be an ancestor of or equal to S, the newest commit that CHANGES the
constant's value. S1 moves two non-comment lines of that scan set at order 3. With the single move in
unit 1's commit at order 1, W is a DESCENDANT of S, the test fails, and the leg — subject `repo`,
guard `None`, so on every bar — reds from this unit's commit through the build's tip.
Two resolutions keep the move singular, and both are build-level rather than unit-level:
the value move is made in a commit that lands at or after this unit's engine commit, still once and
still not written by this unit; or the units land in an order that puts every engine edit before it.
Liveness: this probe can return the other answer. An endpoint comparison would be satisfied by a bump
anywhere in the range, and the file's own header at `:21` records that first cut being reproduced and
refuted — so "clean" is a verdict the mechanism is capable of printing here, and it does not.
Either way this unit's obligation is unchanged: no bump.
...
Three hits are: `kit-dogfood-parity.PAIRS` on the `build-method` dossier, which IS the seam S2's
...
existing per-class cap table, and no new mechanism is written. Two recall hits were STALE against
