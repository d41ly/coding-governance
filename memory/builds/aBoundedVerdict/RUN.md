...
2026-08-19T14:34:17Z decision · item drift-audit's non-terminal-spec-citation signal is unsatisfiable for any multi-unit build that cites its ids in code · reason Measured 10 against a shrink-only pin of 2. Product code cites the unit ids it implements; a spec reaches CLOSED only by landing; the bar must be green to land. So the signal is red for the whole life of any such build and clears only after the thing it blocks. Options seen: stop citing ids in shipped code, which loses the strongest provenance this repo has; add a signal state for 'cited by the diff that implements it'; or raise the pin, which is not available since it is shrink-only. Not mine to choose - it is a gate another build owns and the pin direction is deliberate.

2026-08-19T15:00:27Z abort · item aBoundedVerdict · reason gates-green is unreachable and the fix is outside this mandate: aPacedTurnstile's run is live at LANDING, LANDING is not terminal, and check 7 counts it against every later run. Overriding gates-green for another run's red is refused on this repo's own record; writing --landed or --abort onto another node's record is not this run's to do. Five units are built, armed and green on the branch and nothing is lost by this terminal - preflight rotates a finished record and starts a fresh run against the same build folder. Aborting rather than holding the record live, because two live runs keep check 7 red for the whole fleet and this run's continued existence was half that blockage.
