...
**Status:** SPECCED · rev-1 · 2026-08-31 · node a · Tier-2 · base 72dff924 · streams tooling · order 10
...
- **AC1** — When the D6 fixture (an ambiguous-position declined slash, `const K = 500`, a block
  comment whose prose contains `const K = 5`, a `chunk(args.findings, Math.ceil(...)) //
  gov:fixed-verifiers` split and a `boundedParallel(groups.map(...), 5)`) is fed to the hook, it
  exits `2`. At the tip, at 1.9, at 1.10 and under both lexer patches it exits `0`.
