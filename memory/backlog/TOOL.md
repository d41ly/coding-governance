...
- TOOL-aNumeralWarden-1 · SPECCED · agent-cap.js never reads the fan-out bound it enforces, so `a.maxVerifiers || 5` in two shipped harnesses lets a caller raise the verifier count past the BINDING cap while the hook and the merge-bar leg both report clean
