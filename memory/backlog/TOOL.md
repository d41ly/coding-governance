...
- TOOL-aNumeralWarden-1 · SPECCED · agent-cap.js never reads the fan-out bound it enforces, so `a.maxVerifiers || 5` lets a caller raise the verifier count past the BINDING cap with every gate green; at rev-3 it also owns the Agent-matcher refusal folded in from TOOL-aUnmannedHelm-1
...
- TOOL-aUnmannedHelm-1 · SPECCED · unit 1 of the unattended-run kit: RUN.md and the hygiene contract that admits it; the build's other six units are unspecced, and it DEPENDS on TOOL-aNumeralWarden-1, which took the agent-cap fold
