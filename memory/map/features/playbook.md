...
[claims]
gate-legs = ["template size gate selftest", "playbook parity", "playbook parity selftest", "playbook render wiring", "charter size", "micro-format definitions", "micro-format gate selftest", "line length", "line-length gate selftest"]
kits = ["playbook"]
git-hooks = []
workflow-scripts = []
skill-engines = []
...
- **The gate has TWO consumers, and only one of them is the playbook.** `tools/gate-legs.json` runs
  the same script over `skills/session-kickoff/SKILL.md`, whose ceiling is DECLARED in
  `tools/template-size-limits.txt` rather than passed in argv. It used to be a POSITIONAL, and the
  positional was load-bearing: it beat the environment, so raising the template's default could not
  move the engine's cap. The declaration therefore OUTRANKS the environment — moving the number out
  of argv without that ordering would have let `MAX_BYTES=999999` lift the engine from 18 KiB to
  48 KiB, silently undoing a property a landed unit built and verified.
...
- **Both ceilings are DECLARED, not hardcoded.** `tools/template-size-limits.txt` holds one row per
  measured file with the value's history beside it, on `READ_PATH_CEILING`'s pattern. Resolution is
  positional, then the declaration, then the environment, then a hard default for a subject the file
  does not name. The declaration is a person's file; the high-water record beside it is the gate's,
  and they are deliberately separate so a `--bump` can never move a ceiling as a side effect.
...
seam: check-template-size.sh subject resolution — reuse for gating ANY file's byte size on the
merge bar without writing a sibling script; extend via one `tools/gate-legs.json` entry naming the
subject alone, one row in `tools/template-size-limits.txt` giving its ceiling and the reason for
it, and one `--bump` to seed its high-water row. The limit is no longer passed positionally —
that spelling was this affordance's own text until the declaration replaced it.
