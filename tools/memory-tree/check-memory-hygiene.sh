...
set -u
KIT_MEMORY_TREE_VERSION=2.70   # gov:kit memory-tree@2.70 — engine identity; set HERE, never from .memory-tree.conf (a project conf must not spoof it)
...
# --- guard beside the tracked-path one below reds a row that recorded nothing. This is the shape
# --- `tools/check-testsuite-counts.sh` already spells for its own registry.
#
...
_UNWAIVED=""
split_debt() { # check-number · findings → sets _UNWAIVED to the failing ones, records the rest
  local _l _p
  _UNWAIVED=""
  [ -n "$2" ] || return 0
...
split_debt 6 "$bad6"; bad6="$_UNWAIVED"
...
split_debt 7 "$bad7"; bad7="$_UNWAIVED"
...
split_debt 8 "$bad8"; bad8="$_UNWAIVED"
...
# registry is actually about. Populated by `split_debt` during checks 6, 7 and 8.
#
