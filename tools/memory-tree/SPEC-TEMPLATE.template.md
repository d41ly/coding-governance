...
- A codebase-map dossier claims EXACT inventory keys, so a sentence saying a dossier claims a path,
  ...
  `{{TOOL_ROOT}}codebase-map/gen_map.py`). A CODE SYMBOL carries an underscore between two letters or
  ...
  (`{{TOOL_ROOT}}codebase-map/map_extractors.py`). The space clause: a token
