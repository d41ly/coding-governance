...
# ---- check 29 - a --plan run may never claim terminality over a build it graded nothing on.
...
if [ -d "$MEMORY_ROOT/builds" ]; then
  ...
  [ "$_pv_seen" -gt 0 ] || fail 29 "check 29 walked no build whose --plan returned a verdict, so a clean result here is about an empty population rather than about the corpus: $MEMORY_ROOT/builds"
  [ -z "$_pv_bad" ] || fail 29 "a build's --plan reports NOT A UNIT rows AND claims every tracked spec is terminal, so a reader picking up work is told a build is finished by a verb that graded nothing on it:$_pv_bad"
fi
