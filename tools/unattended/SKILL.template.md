...
{{KEEPALIVE_INTERVAL}} — and keep the id it returns, because `--preflight` refuses without it.
...
5. **If the README carries `asks:`, this run is pointed at ASKS.** Read
   `{{MEMORY_ROOT}}/guides/UNATTENDED-ASKS.md` whole before the first roster row: it is the half of
...
6. **A pre-flip BASE is parked, never relocated.** When preflight's notice says `BACKLOG_MODE`
   ...
   backlogs, park the run with the recipe `{{MEMORY_TREE_DIR}}/migrate_backlog.py --recipe` prints.
...
1. **Hand the value to preflight as typed** — the ids, the ids a prompt names, or the filing home's
   ...
   ```bash
   bash {{KIT_DIR}}/unattended.sh --preflight "<the value>" --keepalive-id <id>
   ```
...
0. **Read `{{MEMORY_ROOT}}/guides/BUILD-METHOD.md` WHOLE**, and read the PLAYBOOK whole. Then read it
   ...
   **Where you are writing the build folder yourself, this path needs `published`**, for the reason
   the prompt path states, and this project declares `{{ANCHOR_SCOPE}}`. Where the owner landed the
...
3. **Write the build folder.** `{{MEMORY_ROOT}}/builds/<slug>/README.md`, with the six required keys
...
```bash
bash {{KIT_DIR}}/unattended.sh --record-piece - --records-root <root> --path <piece> --leg <name> --verdict PASS --run <label>
bash {{KIT_DIR}}/unattended.sh --record-set - --records-root <root> --leg <name> --verdict PASS --run <label> --set <hash,hash>
```
...
- **Record WHAT each build pass was handed**, so "which instructions produced this diff" has an
  ...
  ```bash
  bash {{KIT_DIR}}/unattended.sh --brief <slug> --unit <unit-id> --path <the brief file>
  ```
...
```bash
bash {{KIT_DIR}}/unattended.sh --status <slug>
bash {{KIT_DIR}}/unattended.sh --resume <slug> --keepalive-id <id>
```
...
have pushed.** Issue `{{RESUME_SCHEDULE_DELETE}}` against `unattended-resume-<slug in lower case>`,
...
bash {{KIT_DIR}}/unattended.sh --attest <slug> --item parked-decisions-surfaced --value "yes, <n> surfaced"
```
...
**This section binds only where `{{TOOL_ROOT}}runlog/runlog.py` exists in this repository.** Test for
...
```bash
test -f {{TOOL_ROOT}}runlog/runlog.py   # exit 0: the kit is here and this section binds; exit 1: skip it
python {{TOOL_ROOT}}runlog/runlog.py record <slug> --write
```
...
```bash
bash {{KIT_DIR}}/unattended.sh --hold <slug> --code <hold-code> --until <condition> \
...
1. `{{RESUME_SCHEDULE_DELETE}}` against the printed NAME. Go on when no task has it — that is the
...
2. `{{RESUME_SCHEDULE_CREATE}}` under that same name, at that exact instant, ONE-SHOT, with the
