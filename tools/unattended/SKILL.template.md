...
`bash {{KIT_DIR}}/unattended.sh --status <slug>` before anything else it does. `--status` is among the
...
has its slug and its run-state file, so every fire runs `bash {{KIT_DIR}}/unattended.sh --status <slug>`
