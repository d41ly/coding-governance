...
$1" ;;
    ...
    _find_build_commit() {
      ...
      for _c in $(GIT rev-list $_ord $_mc $_range 2>/dev/null); do
        ...
        # FROM THE CACHE, already tokenised and already space-padded on both ends. A cached subject
        # is never the empty string — an empty subject caches as two spaces — so an empty read is a
        # MISS and nothing else, and a miss falls back to the pair of processes this cache replaced
        # rather than reading as "this commit does not name the id", which would grade the unit
        # unbuilt and report a clean bill. Every commit either window walks is reachable from HEAD
        # (`base..HEAD` by definition, and the pre-anchor anchor is `<first>^` where `<first>` is
        # itself HEAD-reachable), so the fallback is unreachable today; it is here so that widening a
        # window later cannot turn a cache miss into a silent pass.
        _subj=${_SUBJ[$_c]-}
        [ -n "$_subj" ] || _subj=" $(GIT log -1 --format=%s "$_c" 2>/dev/null | tr -c 'A-Za-z0-9-' ' ') "
        case "$_subj" in *" $id "*) ;; *) continue ;; esac
