...
Rendered from `{{KIT_DIR}}/SKILL.template.md` by `adopt-drift-audit.sh`. Edit the template, re-render;
...
```bash
python {{KIT_DIR}}/drift_report.py
```

```
python {{KIT_DIR}}/drift_report.py --json     # full detail per signal
...
Gateable signals carry a **shrink-only pin** in `{{KIT_DIR}}/drift_signals.py`, seeded at measured
