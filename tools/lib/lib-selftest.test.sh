...
# drive <name> <body> -> writes $TMP/<name>.out, returns the harness's exit status
drive() {
  local name=$1 body=$2
  { printf 'set -u\n. "%s"\n' "$LIB"; printf '%s\n' "$body"; } > "$TMP/$name.sh"
...
run_arms t"
SELFTEST_INNER_WIDTH=1 drive w1 "$BODY"
SELFTEST_INNER_WIDTH=4 drive w4 "$BODY"
...
drive wedge "SELFTEST_ARM_TIMEOUT=3
$FIX
