...
# `build_merged_returns <items-json> [judged]`: four lenses of twelve findings each are ids 1-48; the skeptic
# double judges ids 1 to `judged` (default 48), confirming the thirteen below and refuting the rest,
# so an id above `judged` comes back UNVERIFIED; the synthesis double returns the given items.
build_merged_returns() {
  node -e '
    ...
    console.log(JSON.stringify({
      ...
      "verify:": { verdicts },
      synth: { path: "r/merged.md", summary: "13 confirmed in 10 items", blockers: 1, highs: 5, items: JSON.parse(process.argv[1]) },
    }))
  ' "$1" "${2:-48}"
}
...
run_merged_build() { # callee RESULT json -> the build harness run over it
  run_wf "$UNITS" "$(printf '{"spec:":%s,"workflow":%s,"audit:record":%s,"dispose:":%s}' \
    "$SPEC_OK" "${1:-null}" "$(rec BOUNDED)" "$MT_DISPOSE")"
}
t2=$(run_wf "$MT_ARGS" "$(build_merged_returns "$MT_MERGED")" "$MT_T2")
au=$(printf '%s\n' "$t2" | sed -n 's/^RESULT //p')
...
t2=$(run_wf "$MT_ARGS" "$(build_merged_returns "$(printf '%s' "$MT_MERGED" | sed 's/,{"severity":"MEDIUM","ids":\[33\]}//')")" "$MT_T2")
au=$(printf '%s\n' "$t2" | sed -n 's/^RESULT //p')
...
t2=$(run_wf "$MT_ARGS" "$(build_merged_returns "$(printf '%s' "$MT_MERGED" | sed 's/"ids":\[20\]/"ids":[20,4]/')")" "$MT_T2")
au=$(printf '%s\n' "$t2" | sed -n 's/^RESULT //p')
...
t2=$(run_wf "$MT_ARGS" "$(build_merged_returns "$(printf '%s' "$MT_MERGED" | sed 's/\[14,26,40\]/[14,26,40,48]/')" 47)" "$MT_T2")
au=$(printf '%s\n' "$t2" | sed -n 's/^RESULT //p')
...
t2=$(run_wf "$MT_ARGS" "$(build_merged_returns "$(printf '%s' "$MT_MERGED" | sed 's/"MEDIUM","ids":\[33\]/"CRITICAL","ids":[33]/')")" "$MT_T2")
au=$(printf '%s\n' "$t2" | sed -n 's/^RESULT //p')
has    "MT a severity outside the closed four leaves its ids unplaced" "$au" "confirmed id(s) 33 sit in no item"
t2=$(run_wf "$MT_ARGS" "$(build_merged_returns "$(printf '%s' "$MT_MERGED" | sed 's/"HIGH","ids":\[1\]/"high","ids":[1]/')")" "$MT_T2")
au=$(printf '%s\n' "$t2" | sed -n 's/^RESULT //p')
