...
description: >-
  ...
  python tools/runlog/runlog.py. Do NOT use for ordinary code search — finding a symbol, a caller,
...
Rendered from `tools/runlog/SKILL.template.md` by `adopt-runlog.sh`. Edit the template and re-render;
a hand-edited copy reds the adopter's `--check` arm.

## Answer in this order

1. **Locate the committed record.** It is
   `memory/builds/<slug>/build/<date>-build-<unit>-runlog-<key>.md`, one file per run,
   ...
   rendered from is `memory/builds/<slug>/RUN.md`, and an earlier run's sits rotated beside
   it as `RUN.<PHASE>.<8 hex>.md`. A build with no record has not had one rendered: say so, and go on.
2. **Build the local model.** `python tools/runlog/runlog.py model <slug> --json --run <n>`, where
...
3. **Print the narration, and only for a WHY.** When the question needs the reason behind an act,
   ...
   leaves `sessions` empty; `python tools/runlog/runlog.py extract --discover --slug <slug>` then
