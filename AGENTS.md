...
- **A kit file names nothing outside itself by literal, and a gate now REFUSES one rather than
  recording it.** Its own kit dir and tool root are DERIVED at run time, and an empty derivation
  is a refusal, not a fallback. A sibling kit — or anything a non-program document must SAY — is
  a token the render channel fills, and an unresolved token is a refusal rather than an emitted
  brace. A replicated policy value is extracted from the one file that owns it, never retyped.
  The reason is deployment: gov's bytes are written VERBATIM, so a literal naming a kit path
  arrives unchanged in a target installed at another prefix and resolves to nothing there.
  Enforced by the carried-prefix arm of the install-prefix gate, which is a BAN and not a
  ratchet as of `TOOL-dRetiredFork-17`: its writer may lower a count and may not add one, so a
  new literal cannot be absorbed by running the remedy the gate prints. A literal that is
  genuinely correct is justified BY HAND in the ban list, in a reason column, in the pass that
  wants it — a row a person wrote is a decision, and a row the script wrote is an accident
  nobody reviewed.
