...
- **Naming is one of those conventions, and it is gateable.** Declare it in `.lexicon.conf`: a CLOSED verb table every function/method definition leads with and a banned type-suffix list. A repo that declares neither has a naming convention it asks people to remember.
