...
- DERIVE the initial table from the corpus, then FREEZE it and mark that a human curated it; a derived table nobody edited is a mirror of the code, which is the one shape a naming gate must not have (§7). Measure every pin against THIS corpus. Declare a COVERAGE MODE per language — parser, probe (incomplete by construction, reported so every run), or explicitly dark — and make an undeclared one a named refusal, never a silent skip.
