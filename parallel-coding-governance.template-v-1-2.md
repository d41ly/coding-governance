...
*Template **v1.2** · last updated 2026-06-23. When you instantiate this in a repo, record the adopted
version in the copy; diff your copy against this source by `§` heading body (ignore filled `{{…}}` + the
deleted Customize block) to pull in improvements. Change history: git.*

<!-- governance-template: v1.2 -->
...
- **Seed** each tag's counter at the family's flat high-water **+ 1** — *every* node uses that SAME seed as its
  tag's start, then increments only within its own tag (tags keep them disjoint). E.g. high-water `ABL-080` →
  node `d` starts `ABL-d081`, node `a` starts `ABL-a081`.
- **Within a single node**, concurrent sessions still coordinate via the `Next free id` marker (§4): read it,
  allocate above it, bump it in the same commit, re-check at merge. Across nodes the tag prefix is the guarantee.
...
- **One in-flight ledger, not prose status.** Prose "not pushed / N ahead" notes rot (they always go stale).
  ...
  | Node | Branch / worktree | Streams touched | IDs claimed | Status |
  |------|-------------------|-----------------|-------------|--------|
  | … | … | … | one token per family, e.g. `ABL-d081…d083, DES-d012` | `in-flight`/`merged`/`pushed:<sha>` |

  Keep cells terse — Status is one of `{in-flight | merged | pushed:<sha>}` plus at most one short clause; full
  narrative belongs in the journal/decision log, never here. **Prune to stop unbounded growth:** stamp the merge
  SHA on a pushed row, and when starting work delete any pushed row already in your history
  (`git merge-base --is-ancestor <sha> HEAD`). With node-scoped IDs this is the *only* coordination artifact you need.
...
§0, §0.5, §1–§6, §8, §10–§13 — is universal core: keep it verbatim (§8's cross-refs in the lifecycle and §6
assume it's present).
